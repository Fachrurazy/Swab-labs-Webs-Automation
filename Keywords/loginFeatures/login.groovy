...
public class login {
	// Method untuk cek apakah elemen terlihat
	@Keyword
	def boolean isElementVisible(TestObject element, String elementName) {
		if (element != null && WebUI.verifyElementVisible(element, FailureHandling.OPTIONAL)) {
			...
			KeywordUtil.markFailed("${elementName} is NOT visible")
			return false
		}
	}
	...
	def void setInputText(TestObject element, String value, String elementName) {
		if (isElementVisible(element, elementName)) {
			WebElement webElement = WebUiCommonHelper.findWebElement(element, 10)
			webElement.clear()  // Hapus teks sebelumnya
			webElement.sendKeys(value)  // Masukkan teks baru
			KeywordUtil.logInfo("Successfully set text for ${elementName}")
		} else {
			KeywordUtil.markFailed("Unable to set text for ${elementName}")
		}
	}
	...
	def void clickButton(TestObject button, String buttonName) {
		if (isElementVisible(button, buttonName)) {
			...
			KeywordUtil.markFailed("Button ${buttonName} is NOT visible")
		}
	}
	...
	def void loginProcess(String username, String password) {
		try {
			...
			setInputText(inputPassword, password, "Password Input Field")
	...
	//  Method untuk verifikasi login sukses
	@Keyword
	def void verifyLoginSuccess() {
		try {
			KeywordUtil.logInfo("=== Verifying Login Use Valid Credential ===")
	...
	//  Method untuk verifikasi login failed
	@Keyword
	def void verifyLoginFailed() {
		try {
			KeywordUtil.logInfo("=== Verifying Login Use Invalid Credential ===")
	...
	//	// Method untuk findtestData
	//	@Keyword
	//	def void getTestDataValue(String filepath, int row, int coloumn) {
